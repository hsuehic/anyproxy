...
function getUserReqHandler(userRule, recorder) {
   ...
   * @param {http.ServerResponse} userRes
   */
  function handle(req, userRes) {
    /*
    note
      req.url is wired
    ...
    if ((req.headers.accept || req.headers.Accept) === 'text/event-stream') {
      try {
        ...
        const recordInfo = {
          ...
          hosts: [urlPattern.host],
          path: urlPattern.pathname,
          method: req.method,
          req,
        ...
        resourceInfoId = recorder.appendRecord(recordInfo);
        const reqClient = http.get(fullUrl, (res) => {
          try {
            res.pipe(userRes);
            const endTime = new Date().getTime();
            recorder.updateRecord(resourceInfoId, {
              ...recordInfo,
              endTime,
          ...
          res.on('data', (chunk) => {
            try {
              console.log('Server response data:', Buffer.concat([chunk]).toString('utf8'));
            } catch (ex) {
              logUtil.error(ex);
            }
          });
          // background socket closed
          res.on('close', () => {
            logUtil.info('Remote server socket closed!');
            userRes.socket.destroy();
          });
          // readable stream read to end
          res.on('end', () => {
            logUtil.info('Remote server response ended!');
            userRes.end();
          })
        });
        reqClient.on('error', () => { userRes.socket.destroy(); });
        reqClient.on('close', () => {
          logUtil.info('Remote socket closed!');
          userRes.socket.destroy();
        });
        req.pipe(reqClient);
        // background socket closed
        req.on('close', () => {
          ...
          reqClient.socket.destroy();
        });
        // readable stream read to end
        req.on('end', () => {
          logUtil.info('Client request ended');
          reqClient.end();
        });
        req.on('data', (chunk) => {
          try {
            console.log('Client data:', Buffer.concat([chunk]));
          } catch (ex) {
            logUtil.error(ex);
          }
        });
...
class RequestHandler {
   ...
   * @param {string[]} config.currentHosts the hosts represent this server
   * @param {string} config.protocol ws or wss
   * @param {object} rule
   * @param {Recorder} recorder
   *
  ...
  constructor(config, rule, recorder) {
    ...
    this.currentHosts = config.currentHosts;
    ...
    reqHandlerCtx.wsHandler = this.getWsHandler.bind(this, userRule, recorder);
   ...
   * @param {object} obj 
   */
  broadcastToWsClient(obj) {
    const data = JSON.stringify(obj);
    for (const client of this.clients) {
      try {
        client.send(data);
      } catch (ex) {
        logUtil.error('Websock error:', ex);
      }
    }
  }
   ...
   * @param {object} wsReq
   */
  getWsHandler(userRule, recorder, wsClient, wsReq) {
    const self = this;
    try {
      let resourceInfoId = -1;
      const resourceInfo = {
      ...
      let proxyWs;
      if (!(this.currentHosts.some(h => h.indexOf(`${serverInfo.hostName}${serverInfoPort}`) > -1) && serverInfo.protocol === this.protocol)) {
      ...
      */
        ...
        // ws connection target this server, we would send record to these client
